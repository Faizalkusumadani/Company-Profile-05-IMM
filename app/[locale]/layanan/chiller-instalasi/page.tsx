...
import { CheckCircle2, ShieldCheck, Zap, Wrench, Check } from "lucide-react";
import {
  checklistPointKeys,
  scopeOfWorkKeys,
  scopeIcons,
  benefitItems,
} from "@/data/data-chiller";

const siteUrl = "https://intisukses-mm.com";
const HERO_IMAGE = "/images/pemasangan.jpeg";
const GALLERY_IMAGE =
  "/images/produk/Variants/Tica/Water-Cooled Screw Chiller - G Series.png";

export async function generateMetadata(): Promise<Metadata> {
  const locale = await getLocale();
  const t = await getTranslations("instalasi-chiller.meta");
  const title = t("title");
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${siteUrl}/${locale}/layanan/chiller-instalasi`,
      siteName: "PT Intisukses Mitratama Mandiri",
      images: [{ url: HERO_IMAGE, width: 1200, height: 630, alt: title }],
      type: "website",
    },
    alternates: {
      canonical: `${siteUrl}/${locale}/layanan/chiller-instalasi`,
      languages: {
        "id-ID": `${siteUrl}/id/layanan/chiller-instalasi`,
        "en-US": `${siteUrl}/en/layanan/chiller-instalasi`,
      },
    },
  };
}

export default async function LayananInstalasiChillerPage() {
  ...
  return (
    <div className="w-full min-h-screen">
      ...
      {/* Kontainer Utama */}
      <main id="layanan-instalasi-main">
        {/* 1. HERO OVERVIEW SECTION                                          */}
        <section
          id="layanan-instalasi-chiller"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2">
                  {checklistPointKeys.map((key) => (
                    ...
                    >
                      ...
                      <span>{t(`instalasi-chiller.${key}`)}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="relative mx-auto w-full max-w-md lg:max-w-none">
                  ...
                  <div className="relative rounded-3xl shadow-2xl">
                    <div className="relative aspect-4/3 sm:aspect-5/4 lg:aspect-square w-full overflow-hidden rounded-2xl bg-neutral-100">
                      <Image
                        src={HERO_IMAGE}
                        alt="Teknisi profesional melakukan instalasi Hepa"
                        fill
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl space-y-4">
              <span className="inline-block text-xs sm:text-sm font-semibold text-imm-blue uppercase tracking-[0.18em]">
                {t("instalasi-chiller.scope_tag")}
              </span>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground tracking-tight">
                {t("instalasi-chiller.scope_title")}
              </h2>
              <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                {t("instalasi-chiller.scope_desc")}
              </p>
            </div>
        ...
        {/* 3. SECTION BENEFITS (Presisi, Kebersihan, Hemat Daya)            */}
        <section
          id="benefits"
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16 sm:space-y-20">
            ...
            <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
                ...
                <div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-5 sm:p-6 flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    <h4 className="text-sm font-bold text-foreground">
                      {t("instalasi-chiller.benefit_tag_notif")}
                    </h4>
                    <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">
                      {t("instalasi-chiller.benefit_content")}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-5">
                <div className="relative aspect-4/3 sm:aspect-5/4 lg:aspect-square w-full rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-gray-200/80 group">
                  <Image
                    src={GALLERY_IMAGE}
                    alt="Proses instalasi dan perancangan sistem chiller plant terstandar"
                    fill
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 text-white space-y-1.5">
                    <span className="inline-flex items-center gap-1.5 rounded-md bg-imm-orange px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-white">
                      {t("instalasi-chiller.benefit_img_badge")}
                    </span>
                    <h3 className="text-base sm:text-lg font-bold text-white">
                      {t("instalasi-chiller.benefit_img_title")}
                    </h3>
                    <p className="text-xs sm:text-sm text-gray-200/90 leading-relaxed">
                      {t("instalasi-chiller.benefit_img_desc")}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* Bagian Bawah: 3 Core Benefits (Presisi, Kebersihan, Hemat Daya) */}
            <div className="border-t border-gray-200/80 pt-12 sm:pt-16">
              <div className="max-w-3xl mb-8 sm:mb-12 space-y-4">
                <span className="inline-block text-xs sm:text-sm font-semibold uppercase tracking-wider text-imm-blue">
                  {t("instalasi-chiller.benefit_card_01")}
                </span>
                <h3 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">
                  {t("instalasi-chiller.benefit_card_02")}
                </h3>
                <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                  {t("instalasi-chiller.benefit_card_03")}
                </p>
              </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="flex items-center justify-between gap-4 mb-6">
                          ...
                          >
                            {t(`instalasi-chiller.benefit_${item.id}_tag`)}
                          </span>
                        </div>

                        {/* Judul Benefit */}
                        <h4 className="text-xl font-bold text-foreground group-hover:text-imm-blue transition-colors duration-200">
                          {t(`instalasi-chiller.benefit_${item.id}_title`)}
                        </h4>

                        {/* Deskripsi Benefit */}
                        <p className="mt-3 text-sm text-gray-600 leading-relaxed">
                          {t(`instalasi-chiller.benefit_${item.id}_desc`)}
                        </p>
                          ...
                          {item.pointKeys.map((key) => (
                            <li key={key} className="flex items-center gap-2.5">
                              ...
                              <span className="font-medium leading-snug">
                                {t(
                                  `instalasi-chiller.benefit_${item.id}_${key}`,
                                )}
                              </span>
                            </li>
                          ))}
                      ...
                      <div className="mt-8 pt-4 border-t border-gray-100 flex items-center justify-between text-xs font-semibold text-imm-blue">
                        <span>
                          {t("instalasi-chiller.benefit_card_footer")}
                        </span>
                        <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                      </div>
                    </div>
